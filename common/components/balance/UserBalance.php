...
class UserBalance extends Balance
{
    ...
    public function execute($userId, $sum, $params = [], $typeOperation): bool
    {
        ...
        try {
            $this->executeDB($userId, $sum, $typeOperation);
            $this->writeJournal([
                'userId' => $userId,
    ...
    protected function executeDB($userId, $sum, $typeOperation): bool
    {
        $sql = '';
        if ($typeOperation == self::TYPE_OPERATION_INCREASE) {
            $sql = 'UPDATE ' . $this->balanceTableName . ' SET `balance` = `balance` + :sum WHERE `id` = :userId';
